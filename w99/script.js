...
async function fetchProducts() {
    try {
        ...
        products.forEach(product => {
            const displayImg = product.image_url || 'logo99.jpg';
            productList.innerHTML += `
...
window.renderNotifications = function() {
    ...
    // 1. Get ALL low stock items (Red Badge) - Priority 1
    let stokRendahItems = allNotifProducts
        .filter(p => p.stok <= 5)
        .map(p => ({...p, type: 'red'}));

    // 2. Get ALL items sorted by newest ID (Blue Badge) - Priority 2
    // Removed the .slice() to ensure every product is processed
    ...
    // 3. Priority Logic: Always process Red cards first
    if (activeFilters.length === 0) {
        // DEFAULT: Merge both lists. STOK RENDAH must come first in the array
        // so it takes badge priority in the deduplication step below.
        itemsToShow = [...stokRendahItems, ...produkBaruItems];
    } else {
        // FILTERED: Respect the category order (Red then Blue)
    ...
    // 4. DEDUPLICATION & RENDERING
    // This ensures every product in your DB appears exactly once.
    ...
    itemsToShow.forEach(item => {
        if (!seenIds.has(item.id_produk)) {
            ...
            const displayImg = item.image_url || 'logo99.jpg';
