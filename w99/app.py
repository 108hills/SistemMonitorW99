...
def get_products():
    try:
        ...
        # NEW: Added "WHERE is_active = TRUE" so deleted items vanish from the dashboard
...
def add_product():
    try:
        ...
        id_user = data.get('id_user', 1) # Grab dynamic user ID
        
        conn = get_db_connection()
        cur = conn.cursor()
        
        # Insert product with dynamic ID
        ...
        # Log the transaction with dynamic ID
        cur.execute('''
...
def update_stock(id_produk):
    try:
        ...
        new_image = data.get('image_url') # Try to get a new image
        
        conn = get_db_connection()
        cur = conn.cursor()
        
        # If an image was sent, update both stock and image
        if new_image:
...
def delete_product(id_produk):
    try:
        ...
        # 1. Log the deletion in the transaction table
        cur.execute('''
            INSERT INTO transaksi (tanggal, jenis_transaksi, jumlah, id_produk, id_user) 
            VALUES (CURRENT_DATE, 'HAPUS PRODUK', 0, %s, %s)
        ''', (id_produk, id_user))
        
        # 2. SOFT DELETE: Instead of deleting, just set is_active to FALSE
